...
def discover_repo_root() -> Path:
    ...
    for candidate in (SCRIPT_DIR, *SCRIPT_DIR.parents):
        if (candidate / ".git").exists():
            return candidate
        if (candidate / "spatial").is_dir() and (candidate / "analysis").is_dir():
            return candidate
    # This fallback keeps the command useful when the maps folder is copied
    # outside the repository and the user supplies the remaining paths.
